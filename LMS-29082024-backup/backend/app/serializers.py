...
class candidatesSerializerOLd(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'college_id',  'students_name','skill_id', 'registration_number', 'gender', 'email_id', 'mobile_number',
                  'department_id', 'year', 'cgpa', 'marks_10th', 'marks_12th', 'marks_semester_wise','is_offered'
...
class candidatesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = candidate_master
        fields = ['id', 'college_id', 'students_name', 'skill_id', 'registration_number', 'gender', 'email_id', 'mobile_number',
                  'department_id', 'year', 'cgpa', 'marks_10th', 'marks_12th','is_offered',
...
class testcandidatemapSerializersupdate(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'test_name', 'question_id', 'student_id', 'college_id', 'dtm_start','dtm_login','dtm_submit','dtm_start_test','status',
                  'dtm_end', 'attempt_count', 'is_camera_on', 'department_id', 'duration',
...
class testcandidatemapSerializers(serializers.ModelSerializer):
    class Meta:
        ...
        'attempt_count','is_camera_on','department_id','duration','year','rules_id','is_active','need_candidate_info', 'total_score', 'avg_mark', 'duration_type', 'dtm_created']
...
class NonDbTestAssignSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id','test_name','question_id','student_id', 'college_id', 'dtm_start','dtm_login','dtm_submit','dtm_start_test','status',
        'dtm_end',
        'attempt_count','is_camera_on', 'duration','rules_id','is_active','need_candidate_info', 'total_score', 'avg_mark', 'duration_type', 'dtm_created']
...
class jobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id','company_name','company_profile','interview_date','year', 'post_name', 'intern_fulltime', 'on_off_campus', 'skill_id',
                  'department_id','college_id', 'cgpa', 'marks_10th', 'marks_12th','job_type',
...
class trainerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id',
            'trainer_name', 'city', 'state', 'qualification', 'experience', 
            'mobile_no', 'email_id', 'skill_id', 'languages_known', 'bank_name', 
            'ifsc_code', 'branch_name', 'account_no', 'resume_url', 'user_name'
        ]
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        user_data = validated_data.pop('user', None)  # Safely pop 'user' data if it exists
    
        # Update fields on the user_profile instance
        instance.role = validated_data.get('role', instance.role)
        instance.college_id = validated_data.get('college_id', instance.college_id)
        instance.save()
    
        # If user_data is provided, update the User fields
        if user_data:
            user = instance.user
