...
from .forms import QuestionForm, QuestionCodeForm, CollegeForm
...
#trainer_master_________________________________________________________
@api_view(['GET'])
...
def get_tests_candidates_map(request):
    try:
        ...
        tests_candidates = tests_candidates_map.objects.filter(deleted=0).select_related(
            ...
            'total_score', 'avg_mark', 'dtm_created'
        )
...
def get_course_content_feedback(request):
    course_set = course_content_feedback.objects.filter(deleted=0)

    course_content_data=[]
    for candidate in  course_set:
        ...
        if candidate.student_id:
            ...
            department_name = student.department_id.department if student.department_id else None
        ...
        course_content_data.append({
              'id': candidate.id,
             # 'course_id': candidate.course_id.course_name,
              'student_id': student_id,
              'topic_id':topic_id,
               'sub_topic':sub_topic,
              'college_id': college_id,
            ...
            'department_name': department_name,
              'dtm_session': candidate.dtm_session,
              'trainer_id': trainer_id,
...
class ExcelImportView_Questions_3107(APIView):
    def post(self, request, format=None):
...
class NonDbTestAssign_0108(APIView):
    def post(self, request):
        test_name = request.data.get('test_name')
        test_type_id = request.data.get('test_type_id')
        question_type_id = request.data.get('question_type_id')
        ...
        for field in decimal_fields:
            print('field: ', field)
            queries.append(Q({f"{field}__isnull": True}) | Q({field: None}) | Q({f"{field}": 0.00}))
...
def serve_image(request, question_id, option):
    try:
        question = question_master.objects.get(id=question_id)
        image_data = None
        if option == 'a' and question.option_a_image_data:
            image_data = question.option_a_image_data
        elif option == 'b' and question.option_b_image_data:
            image_data = question.option_b_image_data
        elif option == 'c' and question.option_c_image_data:
            image_data = question.option_c_image_data
        elif option == 'd' and question.option_d_image_data:
            image_data = question.option_d_image_data
        
        if image_data:
            response = HttpResponse(image_data, content_type='image/png')
            response['Content-Disposition'] = f'inline; filename={question_id}_{option}.png'
            return response
        else:
            return HttpResponse(status=404)  # Not Found if no image data
    except question_master.DoesNotExist:
        return HttpResponse(status=404)  # Not Found if question does not exist
...
def get_question(request, question_id):
    ...
    response_data = {
        ...
        'option_c': question.option_c,
        'option_d': question.option_d,
        'option_a_image_url': base_url + reverse('serve_image', args=[question_id, 'a']) if question.option_a_image_data else None,
        'option_b_image_url': base_url + reverse('serve_image', args=[question_id, 'b']) if question.option_b_image_data else None,
        'option_c_image_url': base_url + reverse('serve_image', args=[question_id, 'c']) if question.option_c_image_data else None,
        'option_d_image_url': base_url + reverse('serve_image', args=[question_id, 'd']) if question.option_d_image_data else None,
    }
...
def test_candidates_answer_view_Com(request):
    ...
    logger.error("Received request data: %s", request.data)
...
def get_distinct_test_name_count_OLD(request, college_id):
    # Perform the query to count distinct test names for the given college_id
...
def get_avg_score_by_department_OLD(request, college_id, dtm_start):
    try:
        # Convert dtm_start to a datetime object
...
def avg_score_by_department_Coding_OLD(request, college_id, dtm_start):
    try:
        # Convert dtm_start to a datetime object
        dtm_start_date = datetime.datetime.strptime(dtm_start, '%Y-%m-%d').date()
...
def get_distinct_test_name_count_today_OLD(request):
    try:
        # Get today's date
        today_date = date.today()
...
def interview_status_count_view(request, college_id, company_id):
    ...
    # Filter interview_schedule based on the students from the specified college
    results = interview_schedule.objects.filter(student_id__in=students, company_id=company_id) \
        .values('status') \
...
def get_interview_master(request):
    try:
        ...
        interviews = interview_master.objects.filter(deleted=0).select_related(
            'company_id',
            'job_id'
        ).values(
            'id',
            company_id=F('company_id__id'),
            company_name=F('company_id__company_name'),
            job_id=F('job_id__id'),
            post_name=F('post_name'),
            dtm_interview=F('dtm_interview'),
            interview_type=F('interview_type')
        )
        
        # Convert the queryset into a list of dictionaries
        interview_data = list(interviews)
...
def get_interview_schedule(request):
    try:
        # Fetch all interview schedules with related data
        interviews = interview_schedule.objects.filter(deleted=0).select_related(
            'company_id',
            'student_id',
            'post_name_id'
        ).values(
            ...
            company_id=F('company_id__id'),
            company_name=F('company_id__company_name'),
            student_id=F('student_id__id'),
            student_name=F('student_id__students_name'),
            post_name_id=F('post_name_id__id'),
            post_name=F('post_name_id__post_name'),
            dtm_interview=F('dtm_interview'),
            status=F('status'),
            interview_round=F('interview_round')
        )
...
def get_interview_result(request):
    try:
        # Fetch all interview results with related data
        interviews = interview_result.objects.filter(deleted=0).select_related(
            'company_id',
            'student_id',
            'post_name_id'
        ).values(
...
def get_colleges(request):
    collegeset = college_master.objects.all()
