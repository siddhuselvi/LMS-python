...
@api_view(['PUT', 'PATCH'])
def delete_trainer_old(request, pk):
...
def get_trainer_by_userName(request, userName):
    try:
        ...
        # Modify the serialized data to include the absolute URI for the resume field
        for trainer in serialized_data:
            if 'resume' in trainer and trainer['resume']:
                trainer['resume'] = request.build_absolute_uri(trainer['resume'])
...
def trainer_list_view(request):
    trainers = trainer_master.objects.all()
...
def trainer_create_view(request):
    if request.method == 'POST':
        form = TrainerMasterForm(request.POST, request.FILES)
        if form.is_valid():
            trainer = form.save(commit=False)

            # Handle the photo binary data correctly
            photo_file = request.FILES.get('photo_file')
            if photo_file:
                trainer.photo = photo_file.read()

            trainer.save()
            # return redirect('trainer-list')
    else:
        form = TrainerMasterForm()

    return render(request, 'trainer_form.html', {'form': form})


@csrf_exempt
def trainer_update_view(request, pk):
    trainer = get_object_or_404(trainer_master, pk=pk)
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if photo_file:
                trainer.photo = photo_file.read()  # Read as binary data
            trainer.save()
            return redirect('trainer-list')
    else:
...
@api_view(['GET'])
def TrainerListAPIView(request):
    try:
        # Fetch trainers with the given username and not deleted
        trainer = trainer_master.objects.filter(deleted=0)
        # Serialize the data
        serializer = trainerSerializer(trainer, many=True)
