...
const Uploadstudentdata = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // Assuming the backend returns a success message in the response
      setErrorMessage(response.data.success || 'Data uploaded successfully');
      setShowError(true);

      // Call handleRoundSelectionChange after a successful upload
      handleRoundSelectionChange(jobIdUpload, selectedOption, selectedCompanyName); // Replace 'CompanyNameHere' with the actual company name  
      ...
      if (error.response) {
        ...
        errorMsg = Array.isArray(errorData)
          ? errorData.map(err => err.message).join(', ')
