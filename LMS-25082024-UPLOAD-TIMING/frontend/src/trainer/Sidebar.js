                ...
                {islmsOpen && isOpen && (
                  <ul className="test-options" style={{ paddingLeft: '15px' }}>
                    <li className={`test-option ${activeMenuItem === 'learningMaterial' ? 'active' : ''}`} onClick={() => handleMenuItemClick('learningMaterial')}>
                      <Link to="/Lms/upload-video" onContextMenu={(e) => e.preventDefault()}>Learning Material</Link>
                    </li>
                  </ul>
                )}
            </div>
          ...
          <div className={`test-section ${isTestOpen ? 'open' : ''}`}>
               ...
               {isTestOpen && isOpen && (
                  <ul className="test-options" style={{ paddingLeft: '15px' }}>
                    ...
                    <li className={`test-option ${activeMenuItem === 'testReport' ? 'active' : ''}`} onClick={() => handleMenuItemClick('testReport')}>
                      <Link to="/test/report" onContextMenu={(e) => e.preventDefault()}>TestReport</Link>
                    </li>
                ...
                <div className="test-header" onClick={() => toggleMenu('database')}>
                  ...
                  <ul className="test-options" style={{ paddingLeft: '15px' }}>
                    <li className={`test-option ${activeMenuItem === 'uploadProfile' ? 'active' : ''}`} onClick={() => handleMenuItemClick('uploadProfile')}>
                      <Link to="/Database/upload-profile" onContextMenu={(e) => e.preventDefault()}>TrainerProfile</Link>
                    </li>
                    {/* <li className={`test-option ${activeMenuItem === 'attendanceSheet' ? 'active' : ''}`} onClick={() => handleMenuItemClick('attendanceSheet')}>
                      <Link to="/Database/Attendance" onContextMenu={(e) => e.preventDefault()}>Attendancesheet</Link>
                    </li> */}
                  </ul>
                )}
            </div>
