...
const Uploadstudent = ({ collegeName, username }) => {
  ...
  const handleUpdate1 = async (e, index) => {
    e.preventDefault();
    ...
    try {
      // Handling updates based on student's properties
      ...
      //const skillId = student.skill_id ? parseInt(student.skill_id) : null;
      const skillIds = Array.isArray(student.skill_id) ? student.skill_id : [];
      console.log('skillIds: ', skillIds);

      const skillId = skillIds.includes('none') ? [] : skillIds.map(id => parseInt(id));
      console.log('skillId: ', skillId);

      // Validation checks for required fields
      if (collegeId === null || isNaN(collegeId) || departmentId === null || isNaN(departmentId) || skillId === null || isNaN(skillId)) {
        throw new Error('Invalid college, department, or skill ID');
      }
      ...
      // Ensuring that valid college, department, and skill were found
      if (!college || !department || !skill) {
        throw new Error('Invalid college, department, or skill');
      }
      ...
      // Refreshing student data after update
      fetchTraineeData();
