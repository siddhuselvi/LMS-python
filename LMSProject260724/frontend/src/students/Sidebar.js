              ...
              <div className={`test-section ${isDatabaseOpen ? 'open' : ''}`}>
                ...
                {isDatabaseOpen && isOpen && (
                  <ul className="test-options" style={{ paddingLeft: '15px' }}>
                    ...
                    <li className={`test-option ${activeMenuItem === 'trainingSchedule' ? 'active' : ''}`} onClick={() => handleMenuItemClick('trainingSchedule')}>
                      <Link to="/Database/trainingschdl" onContextMenu={(e) => e.preventDefault()}>Training Schedule</Link>
                    </li>
                  </ul>
                )}
              </div>
            </li>
            {/* Uncomment and adjust the following section if you want to add more menus */}
            {/* <li className={`test-option ${activeMenuItem === 'companystatistic' ? 'active' : ''}`} onClick={() => handleMenuItemClick('companystatistic')}>
              <img src={CompanyIcon} alt="Company Statistic" className="icon-image" />
              <Link to="/companystatistic" onContextMenu={(e) => e.preventDefault()}>Company Statistic</Link>
            </li> */}
          </ul>
        </nav>
      </div>
