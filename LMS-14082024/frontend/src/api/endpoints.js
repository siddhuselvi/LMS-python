...
export function getTestAnswerMapApi(username) {
...
export async function addTrainerApi(trainerData) {
  ...
  return await axios.post(`${API_URL}/api/trainers/create/`, {
...
export function updateTestcadidateApi_updatedDatabse(id) {
  return axios.patch(`${API_URL}/api/testcandidate/${id}/updatedDatabase/`)
    .then(response => response.data)
    .catch(error => {
      console.error('Error updating tests:', error);
      throw error;
...
export function getEligibleStudentCountApi() {
  return axios.get(`${API_URL}/api/eligible-students/count/`)
    .then(response => response.data)
}
...
export async function update_MCQ_images_API_NEW(id, test) {
  ...
  return axios.post(`${API_URL}/question_master/${id}/update/`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
      'X-CSRFToken': csrftoken
    }
...
export async function getTrainerByUsername(username) {
  return await axios.get(`${API_URL}/api/trainer/${username}/`)
    .then(response => response.data)
}
