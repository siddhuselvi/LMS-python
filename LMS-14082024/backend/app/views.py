...
from .models import question_master, question_master_temp, question_paper_master
...
def get_tests_candidates_map(request):
    try:
        ...
        tests_candidates = tests_candidates_map.objects.filter(deleted=0).select_related(
            ...
            'dtm_start', 'dtm_end', 'attempt_count', 'is_actual_test', 'is_active',
...
def get_tests_candidates_map_Reports(request):
    try:
        ...
        tests_candidates = tests_candidates_map.objects.filter(deleted=0).select_related(
            ...
            'attempt_count', 'is_actual_test', 'is_active', 'duration', 'duration_type', 'year', 'rules_name', 'instruction', 
...
@api_view(['PUT', 'PATCH'])
def update_testcandidatemap_status_login(request, pk):
    try:
        print("Entering Function..")
        tests_candidates = tests_candidates_map.objects.get(id=pk)
        print("tests_candidates: ", tests_candidates)
    ...
    tests_candidates.dtm_login = currentDate 
    tests_candidates.status = "Database Updated"  # Assuming you want to set status to this string
...
def get_tests_candidates_answer(request):
    ...
    # Fetch data using ORM with related fields
    test_candidate_answer_data = tests_candidates_answers.objects.filter(deleted=0, student_id__user_name=username).select_related('student_id', 'question_id').order_by('-id').values(
        'id',
        'student_id__id',  # Fetch student ID
        'question_id__id',  # Fetch question ID
        'test_name',
...
class ExcelImportView_CandidateLAST(APIView):
    def post(self, request, format=None):
        ...
        login_data = df_valid[['email_id', 'user_name', 'password', 'college_id', 'role']]
...
class ExcelImportView_Candidateuser(APIView):
    def post(self, request, format=None):
        ...
        # Get existing users
        existing_users = set(login.objects.filter(user_name__in=login_data['user_name']).values_list('user_name', flat=True))
...
class ExcelImportView_Questions(APIView):
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            # Add detailed row and column information for validation errors
            detailed_errors = []
            for idx, error in enumerate(serializer.errors.get('non_field_errors', [])):
                detailed_errors.append(f"Row {idx + 1}: {error}")

            for idx, record in enumerate(records):
                for field, messages in serializer.errors.items():
                    if isinstance(messages, list):
                        for message in messages:
                            detailed_errors.append(f"Row {idx + 1}, Column '{field}': {message}")
...
class NonDbTestAssign(APIView):
    def post(self, request):
        ...
        queries = [
            ...
            Q(email_id__isnull=True) | Q(email_id=''),
            Q(mobile_number__isnull=True) | Q(mobile_number=''),
            Q(history_of_arrears__isnull=True) | Q(history_of_arrears=0),
            Q(standing_arrears__isnull=True) | Q(standing_arrears=0),
            Q(number_of_offers__isnull=True) | Q(number_of_offers=0),
            Q(text__isnull=True) | Q(text=''),
            ~Q(user_name__isnull=True) & ~Q(user_name=''),
        ]
        ...
        for field in decimal_fields:
            print('field: ', field)
            queries.append(Q(**{f"{field}__isnull": True}) | Q(**{field: None}) | Q(**{field: 0.00}))
...
def get_candidate_login(request):
    ...
    # Fetch the candidates and annotate the password from the login table
...
def test_candidates_answer_view_Com(request): 
    ...
    try: 
        ...
        compiler_output.objects.filter(student_id=stu_id).update(output=output) 

        print('Successfully updated output')

        # Return a success response
        return Response({'message': 'Successfully updated output'}, status=200)
...
def test_candidates_answer_view_Submit_Com(request, format=None): 
    ...
    try: 
        ...
        result = question.mark if question.answer == ans else round((fuzz.ratio(question.explain_answer.strip(), code.strip()) / 100) * question.mark) 
        print('Result: ', result)
...
def get_tests_candidates_map_MCQ(request, user_name):
    # Create a cache key based on the user_name
    cache_key = f'mcq_test_{user_name}'
    
    # Try to get the data from the cache
    test_candidate_map_data = cache.get(cache_key)
    
    if not test_candidate_map_data:
        ...
        # Cache the data with a timeout of 1 hour (3600 seconds)
        cache.set(cache_key, test_candidate_map_data, timeout=3600)
...
def get_tests_candidates_map_Coding(request, user_name): 
    cache_key = f'upcomming_test_{user_name}'
    upcoming_test = cache.get(cache_key)

    current_date = datetime.now().date() 
    if not upcoming_test:
        ...
        upcoming_test = list(tests_candidates)
        cache.set(cache_key, upcoming_test, timeout=3600)
  

    # Return cached or freshly generated data 

    return Response(upcoming_test) 
...
def get_database_candidate(request):
    try:
        # Fetch candidates with related college and department, and prefetch skills, filtering by is_database=True
        candidatelist = candidate_master.objects.filter(deleted=0, is_database=True).select_related('college_id', 'department_id').prefetch_related('skill_id').values(
            'id', 
            'college_id__college',  # Get college name
            'students_name', 
            'user_name', 
        ...
        for candidate in candidatelist:
            ...
            fields_to_check = ['students_name', 'registration_number', 'gender', 'mobile_number']
...
def get_nondb_candidate(request):
    # Get the list of user_names from CandidateMaster where is_database is False
    subquery_user_names = candidate_master.objects.filter(is_database=False).values_list('user_name', flat=True)
    
    # Filter Login entries where user_name is in the list from the subquery
    nondatabase_candidates = login.objects.filter(user_name__in=subquery_user_names).values('user_name', 'password')
...
@csrf_exempt
def update_eligible_studentold(request, student_id):
    student = get_object_or_404(eligible_student_list, id=student_id)

    if request.method == 'POST':
        form = EligibleStudentListForm(request.POST, request.FILES, instance=student)
        if form.is_valid():
            student = form.save(commit=False)
            if 'announcement_image' in request.FILES:
                student.announcement_image = request.FILES['announcement_image'].read()
            else:
                student.announcement_image = None

            student.save()
            logger.info("Eligible Student Updated Successfully")
            return HttpResponse("Eligible student updated successfully")
    else:
        form = EligibleStudentListForm(instance=student)

    return render(request, 'update_eligible_student.html', {'form': form})



@csrf_exempt
def update_eligible_student(request, student_id):
    student = get_object_or_404(eligible_student_list, id=student_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Check if an announcement_master entry exists
            announcement_entry = announcement_master.objects.first()
            if not announcement_entry:
                announcement_entry = announcement_master()

            # Update or create the announcement_master entry
            announcement_entry.announcement = student.announcement
            if 'announcement_image' in request.FILES:
                announcement_entry.announcement_image = student.announcement_image
            announcement_entry.save()

            return HttpResponse("Eligible student updated and announcement created/updated successfully")
    else:
        form = EligibleStudentListForm(instance=student)
...
class UpdateEligibleStudentListView(APIView):
    def post(self, request, format=None):
        ...
        for _, row in df.iterrows():
            ...
            # Find all candidate_master records with the registration_number
            candidates = candidate_master.objects.filter(registration_number=reg_no)
            if candidates.exists():
                for candidate in candidates:
                    eligible_ids = eligible_student_list.objects.filter(students_id=candidate.id)
                    if eligible_ids.exists():
                        eligible_ids.update(round_of_interview=round_of_interview)
                    else:
                        # Optionally handle case where no eligible student list record exists
                        print(f"No eligible student list record found for candidate with reg_no {reg_no}")
                        continue
            else:
                print(f"No candidate found with reg_no {reg_no}")
                continue
...
@api_view(['GET'])
def get_total_marks(request, student_id, test_name):
