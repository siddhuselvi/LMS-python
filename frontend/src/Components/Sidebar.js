      ...
      <div style={{marginTop:"20px"}}>
        <nav className="sidebar-nav">
          <ul>
            ...
            <li>
              <div className="test-section" style={{ width: '100%' }}>
                <div className="test-header" onClick={togglecmpyMenu}>
                  <img src={CompanyIcon} alt="Company Statistic" className='icon-image' />
                  <Link>CompanyStatistic</Link>
                  {iscmpyOpen && isOpen && (
                    <img src={Downarrow} alt="Down Arrow" className="images15" />
                  )}
                  {!iscmpyOpen && isOpen &&(
                    <img src={Downarrow} alt="Down Arrow" className="images15 rotate-up" />
                  )}

                </div>
                {iscmpyOpen && isOpen &&(
                  <ul className="test-options" style={{ paddingLeft: '20px' }}>
                    <li className={`test-option ${activeMenuItem === 'allocateAdmin' ? 'active' : ''}`} onClick={() => {handleMenuItemClick('allocateAdmin'); toggleSubmenu();}}>

                      <Link to="/Companystatistic/allocate">AllocateAdmin</Link>
                    </li>
                  </ul>
                )}
              </div>
