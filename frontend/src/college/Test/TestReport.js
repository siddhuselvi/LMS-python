...
const TestReports = ({ collegeName }) => {
  ...
  useEffect(() => {
    getTestCandidates();
  }, [test_name, collegeName]); // Run once on component mount
  ...
  const filterCandidates = () => {
    return testCandidates.filter(candidate => {
      if (candidate.college_id !== collegeName) {
        return false;
      }
        ...
        <div className='table-responsive'>
          <table className="product-table">
            <thead className="table-thead">
              <tr>
                <th>Test Name
                  <select value={filters.test_name} onChange={(e) => handleFilterChange(e, 'test_name')} style={{ backgroundColor: 'white', borderRadius: '5px', width: "170px", textAlign: "left" }}>
                ...
                <th>Candidate
                  <select value={filters.student_name} onChange={(e) => handleFilterChange(e, 'student_name')} style={{ backgroundColor: 'white', borderRadius: '5px', width: "140px" }}>
                    <option value="">All</option>
            ...
            <tbody>
              {currentData.map((item, index) => (
                ...
                {/*}  <td>{item.college_id}</td>*/}
                  ...
                  <td>{formatDate(item.dtm_start)}</td>
                  <td>{formatDate(item.dtm_end)}</td>
