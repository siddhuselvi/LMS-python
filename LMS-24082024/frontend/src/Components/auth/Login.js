...
function Login({ onLogin }) {
    ...
    useEffect(() => {
        // Load stored credentials if rememberMe was set
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const response = await getLoginView_API(username, password);

            // Destructure response directly
            const { role, college_id, college_name } = response;

            setUserRole(role);
            setCollegeName(college_name);
            setInstitute(college_id);
