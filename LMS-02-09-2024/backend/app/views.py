...
from .forms import QuestionImportForm, TrainerMasterForm
...
from .forms import ScreenshotsForm
...
@api_view(['GET'])
def get_tests_Reports_OLD(request):
    tests_candidates = tests_candidates_map.objects.filter(deleted=0).select_related(
        'rules_id', 'department_id', 'question_id', 'student_id', 'college_id'
    )
...
class testcandidateanscreateAPIView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        # Extract relevant fields from the request data to identify unique records
        student_id = request.data.get('student_id')
        question_id = request.data.get('question_id')
        test_name = request.data.get('test_name')
        
        # Check if an entry with the same student_id, question_id, and test_name exists
        existing_entry = tests_candidates_answers.objects.filter(
        ...
        if existing_entry:
            # If the entry exists, update it with the new data
    ...
    def post(self, request, format=None):
       ...
       # new_login_data = login_data[~login_data['user_name'].isin(existing_users)]
        ...
        if not new_login_data.empty:
            ...
            # Avoid validation errors by handling existing user names before calling the serializer
...
def get_candidate_login(request):
    ...
    candidatelist = candidate_master.objects.filter(deleted=0).annotate(
        ...
        'id', 'user_name', 'students_name', 'password', 'college_id__college'
    )
...
class jobcreateAPIView(generics.CreateAPIView):
    ...
    def filter_candidates(self, job):
        ...
        filters = Q(deleted=0) & Q(is_offered=False) 
...
def test_candidates_answer_view_Submit_Com(request, format=None): 
    ...
    try: 
        # Condition 1: Check if output matches question_answer exactly
        if question_answer.strip() == output.strip():
            result = question_mark
            print('output mark: ', result)
        else:
            # Check for errors in output
            has_errors = 'error' in output.lower() or 'exception' in output.lower()  # Adjust based on error keywords
            
            # Fuzzy matching for code similarity
            fuzzy_score = fuzz.ratio(explain_ans.strip(), code.strip())
            result = round((fuzzy_score / 100) * question_mark)
            print('fuzzy mark: ', result)
            
            # Reduce marks further if there are output errors
            if has_errors:
                result = round(result - 1)  # For example, cut marks by half if there's an error
                print('hash error result: ', result)
            
        # Ensure that result does not exceed question_mark
...
@api_view(['GET'])
def get_eligible_students_count(request, clg_id):
    # Aggregate the eligible students by job_id and count the number of students
    eligible_students_data = eligible_student_list.objects.filter(students_id__college_id=clg_id).values(
        'job_id__id',  # Job ID
...
def get_nondb_candidate(request):
    # Create a subquery to fetch the dtm_upload from candidate_master based on the user_name
    subquery_dtm_upload = candidate_master.objects.filter(
        user_name=OuterRef('user_name')  # Assuming user_name in candidate_master is directly related to user_name in login
    ).values('dtm_upload')[:1]  # Limiting to one record since we expect one match per user_name

    # Filter login entries where user_name exists in candidate_master with is_database=False
    nondatabase_candidates = login.objects.filter(
        user_name__in=candidate_master.objects.filter(is_database=False).values_list('user_name', flat=True)
    ).annotate(
        dtm_upload=Subquery(subquery_dtm_upload)
    ).values('user_name', 'password', 'dtm_upload').order_by('-id')  # Order by id in descending order
...
class UpdateEligibleStudentListView(APIView):
    def post(self, request, format=None):
        ...
        try:
            # Fetch job type from job_master based on job_id_value
            job = job_master.objects.get(id=job_id_value)
            job_type = job.job_type  # Assuming 'job_type' is a field in job_master (IT or Core)
            ...
            with transaction.atomic():
                ...
                eligible_students = eligible_student_list.objects.filter(
                    job_id=job_id_value,
                    students_id__registration_number__in=[s['reg_no'] for s in students_to_update]
                )
...
def distinct_job_companies(request):
    ...
    distinct_companies = job_master.objects.values('company_name').distinct()
...
def get_student_announcement(request, students_id):
    ...
    student_entries = eligible_student_list.objects.filter(students_id=students_id)
...
def get_eligible_student_Reports(request):
    ...
    valid_round_options = ['Registered', 'Interview Date', 'Preplacement Talk', 'Round1', 'Round2', 'Offer']
...
def getRoundOfInterviews_API(request):
    try:
        ...
        rounds = eligible_student_list.objects.values_list('round_of_interview', flat=True).distinct()
...
def get_max_score_by_department_placement(request, college_id):
    ...
    try:
        ...
        test_names = test_master.objects.filter(test_type_id__in=type_ids).values_list('test_name', flat=True)
...
def  get_notification_count(request):
    # Adjust the filter based on your requirement
...
class StudentRequestListAPIView(generics.ListAPIView):
    queryset = student_request.objects.all()
    serializer_class = studentRequestSerializer
...
class UpdateStatusAPIView(APIView):
    def put(self, request, student_id):
        ...
        student_requests = student_request.objects.filter(student_id=student_id)
...
def get_trainers_report(request):
    # Use select_related to fetch all fields from course_schedule model based on course_schedule_id
    trainer_report_list = trainers_report.objects.filter(deleted=0).select_related('course_schedule_id').values(
        'id', 
        ...
        'remarks',
        # Fields from course_schedule model
        'course_schedule_id__college_id__college', 
        'course_schedule_id__department_id__department', 
        'course_schedule_id__year', 
        'course_schedule_id__topic_id__topic', 
         'course_schedule_id__topic_id__sub_topic', 
        'course_schedule_id__student_id__students_name',
...
def update_trainer(request, trainer_id):
    ...
    trainer = get_object_or_404(trainer_master, id=trainer_id)

    if request.method == 'POST':
        # Bind form with POST data and FILES data (for uploads)
        ...
        if form.is_valid():
            ...
            if 'photo' in request.FILES:
                ...
                trainer.photo = None  # Clear the photo field if no file is uploaded
            
            # Handle the resume file (FileField will handle file saving)
        ...
        # Populate the form with the existing trainer data
