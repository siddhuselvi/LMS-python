...
const AttendCodeTest = ({ collegeName, username, isSidebarOpen, disableSidebar, enableSidebar }) => {
    const { questionIdCon,
        ...
        setCustomInputCom, 
        outputWindowCom, 
        setOutputWindowCom } = useTestContext();
    ...
    const handleGoForTest = async (selectedCandidateId) => {
        ...
        */}
        ...
        try {
            ...
            console.log('Go For Test totalResults: ', totalResults);
    ...
    useEffect(() => {
        // Calculate if all questions are completed, only if there are questions
        const allQuestionIds1 = questions.map(question => question.id);
        //console.log('allQuestionIds1: ', allQuestionIds1);

        const allQuestionsCompleted = questions.length > 0 && allQuestionIds1.every(id => completedQuestions.includes(id));
        //console.log('allQuestionsCompleted: ', allQuestionsCompleted);
        //console.log('isTotalScoreUpdated: ', isTotalScoreUpdated);
        //console.log('selectedCandidateId: ', selectedCandidateId);
    
        if (allQuestionsCompleted && !isTotalScoreUpdated) {
            updateTotalScoreTestcandidateApi(selectedCandidateId, { total_score: totalResults })
                .then(response => {
                    console.log('Total score updated successfully:', response);
                    setIsTotalScoreUpdated(true); // Mark total score as updated
                })
                .catch(error => {
                    console.error('Error updating total score:', error);
                });
        }
    }, [questions, completedQuestions, isTotalScoreUpdated, selectedCandidateId, totalResults]);
