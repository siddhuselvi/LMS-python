...
{!isAptitudeOpenLMS && (
      ...
      <li className={`test-option ${activeMenuItem === 'softskills' ? 'active' : ''}`} onClick={() => handleMenuItemClick('softskills')}>
        ...
        <Link to='/Lms/upload-video' onClick={() => { handlePre('Softskills'); handleSkillTypeLMS(''); toggleSubmenu(); }} onContextMenu={(e) => e.preventDefault()}>Softskills</Link>
      ...
      <li className={`test-option ${activeMenuItem === 'map-lms' ? 'active' : ''}`} onClick={() =>{ handleMenuItemClick('map-lms'); toggleSubmenu(); }} >

        <Link to="/lms/map/" onClick={() => { handlePre('map-lms'); toggleTechMenuLMSmap(); }} onContextMenu={(e) => e.preventDefault()}>Map LMS</Link>

      </li>
      <li className={`test-option ${activeMenuItem === 'stufeedback' ? 'active' : ''}`} onClick={() =>{ handleMenuItemClick('stufeedback'); toggleSubmenu(); }} >

<Link to="/stufeedback/" onClick={() => { handlePre('stufeedback'); toggleTechMenuLMSmap(); }} onContextMenu={(e) => e.preventDefault()}>Studentfeedback</Link>

</li>
<li className={`test-option ${activeMenuItem === 'Trainerfeedback' ? 'active' : ''}`} onClick={() =>{ handleMenuItemClick('Trainerfeedback'); toggleSubmenu(); }} >

<Link to="/Trainerfeedback/" onClick={() => { handlePre('Trainerfeedback'); toggleTechMenuLMSmap(); }} onContextMenu={(e) => e.preventDefault()}>TrainersReport</Link>
  ...
  const QuestionsOptions = () => (
    <ul className='test-options' style={{ paddingLeft: '20px' }}>
      ...
      {/*}  <li className={`test-option ${activeMenuItem === 'binary' ? 'active' : ''}`} onClick={() => {handleMenuItemClick('qp-paper'); toggleSubmenu();}}>

        <Link to='/binary/to/image' onClick={() => { handleTestType('Coding Test'); }}>Images</Link>
                          ...
                          {isMCQOpen && isOpen && (
                            <ul className="test-options" style={{ paddingLeft: '15px' }}>
                              ...
                              <li className={`test-option ${activeMenuItem === 'companySpecific' ? 'active' : ''}`} onClick={() => handleMenuItemClick('companySpecific')}>
                                ...
                                <Link onContextMenu={(e) => e.preventDefault()} onClick={() => { handleMcqOp('Company Specific'); toggleComMenu(); }}>CompanySpecific</Link>
